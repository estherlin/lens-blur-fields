...
"""
Given a sharp image, render a synthetic image with a desired lens position blur kernel
"""

utilities_directory = os.path.join(os.path.dirname(os.path.dirname(__file__)), "util")
sys.path.insert(0, utilities_directory)
from helper import make_results_dir
from printarr import printarr

# Load instant-ngp library
try:
...
if __name__ == '__main__':
    ...
    printarr(patterns, blurry, sharp, lens_positions)
    printarr(sharp)
    data.close()

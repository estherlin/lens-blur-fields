# @brief: Converts a directory of DNG files to PNG files using rawpy and imageio
# @author: @estherlin
# @date: 2020-05-05
# @usage: python dng2png.py --input_dir path/to/dngs --output_dir path/to/jpgs
...
def convert_dng_to_png(input_dng, output_jpg):
...
def process_directory(input_dir, output_dir):
    ...
    print(len(dng_files), "DNG files found in", input_dir)
    for input_dng in dng_files:
        output_jpg = os.path.join(output_dir, os.path.splitext(os.path.relpath(input_dng, input_dir))[0] + '.png')
        os.makedirs(os.path.dirname(output_jpg), exist_ok=True)
        convert_dng_to_png(input_dng, output_jpg)
